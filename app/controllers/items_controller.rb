...
before_action :set_item, except: [:index, :new, :create, :get_category_children, :get_category_grandchildren]
  ...
  def edit
    ...
    #  実験中の記述
    @edit_category_children = Category.find_by(name:@item.parentcategory, ancestry:nil).children
    @edit_category_grandchildren = Category.find_by(id:@item.childcategory).children
    # 親カテゴリーが選択された後に動くアクション
    def edit_category_children
      #選択された親カテゴリーに紐付く子カテゴリーの配列を取得
      @edit_ategory_children = Category.find_by(name: "#{params[:parent_name]}", ancestry: nil).children
    end

    # 子カテゴリーが選択された後に動くアクション
    def edit_category_grandchildren
      #選択された子カテゴリーに紐付く孫カテゴリーの配列を取得
      @edit_category_grandchildren = Category.find("#{params[:child_id]}").children
    end
